...
Below you can find first few lines of minimal data files presented in form of data frame. As you can see all of them have heades and colnames. To find them localization please run `find.package("OmicsON")` after package loading `library(OmicsON)`.
...
knitr::kable(lipidomicsInputData[1:5, 1:7], caption = "Lipidomisc data")
...
```{r, echo=TRUE, results='asis'}
as.character(decoratedByStringBaseOnEnsembleIds[5, "root"])
as.character(decoratedByStringBaseOnUniProtIds[5, "root"])
```
```{r, echo=TRUE, results='asis'}
decoratedByStringBaseOnEnsembleIds[5, "ensembleIds"][[1]]
decoratedByStringBaseOnUniProtIds[5, "uniProtIds"][[1]]
```

```{r, echo=TRUE, results='asis'}
decoratedByStringBaseOnEnsembleIds[5, "stringGenesSymbolsNarrow"][[1]]
decoratedByStringBaseOnUniProtIds[5, "stringGenesSymbolsNarrow"][[1]]
```
...
Below we presented the list of problems thet could be solved in the same way, by manipulation of xCutoff and yCutoff arguments. It is important to konw that sometimes you have to increase cutoff. Best practice is start from 1 and go down by 0.1 ticks on both. If it not help then try this approach to one cut of, for example change x with frozen y value.
